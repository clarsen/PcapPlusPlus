...
function HELP {
   ...
   echo "--default             --Setup PcapPlusPlus for FreeBSD without PF_RING or DPDK. In this case you must not set --pf-ring or --dpdk"
   echo ""
...
# copy the basic FreeBSD platform.mk
...
# add the FreeBSD definitions to PcapPlusPlus.mk
...
sed -i '.bak' -e "1s|^|PCAPPLUSPLUS_HOME := $PWD\
\
|" $PCAPPLUSPLUS_MK
...
if (( $COMPILE_WITH_PF_RING > 0 )) ; then
   ...
   sed -i .bak -e "2s|^|PF_RING_HOME := $PF_RING_HOME\
\
|" $PCAPPLUSPLUS_MK
fi
...
if (( $COMPILE_WITH_DPDK > 0 )) ; then
   ...
   sed -i .bak -e "2s|^|USE_DPDK := 1\
\
|" $PCAPPLUSPLUS_MK

   # set DPDK home to RTE_SDK variable in PcapPlusPlus.mk
   sed -i .bak -e "2s|^|RTE_SDK := $DPDK_HOME\
\
|" $PCAPPLUSPLUS_MK
   ...
   sed -i .bak -e "s|###RTE_SDK###|$DPDK_HOME|g" setup-dpdk.sh
...
sed -i .bak -e "s|{{INSTALL_DIR}}|$INSTALL_DIR|g" mk/install.sh && rm mk/install.sh.bak
chmod +x mk/install.sh

cp mk/uninstall.sh.template mk/uninstall.sh
sed -i .bak -e "s|{{INSTALL_DIR}}|$INSTALL_DIR|g" mk/uninstall.sh && rm mk/uninstall.sh.bak
chmod +x mk/install.sh
